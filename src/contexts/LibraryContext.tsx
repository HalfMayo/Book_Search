import {useReducer, useEffect, createContext, useContext, useCallback, ReactNode, Dispatch} from 'react'
...
interface LibraryContextProps {
    ...
    results: BookProps[],
    getBooks: (topic:string, index: number) => void,
    query: string,
    pageNumber: number,
...
export type Actions = TabActive | Toggle | ToRead | Read | LoadResults | RemoveToRead | PageFirst | PageIncrement | PageDecrement | SetQuery | BookOpen | ListAddNotes | BookAddNotes
    ...
    switch(action.type) {
        ...
        case "toggle":
            if(state.isActive === 1) {
                ...
                return {...state, todo: todosRemaining, completed: [...state.completed, todoDone[0]]};
                ...
                return {...state, completed: completedRemaining, todo: [...state.todo, completedUndone[0]]}
            } else return state;
        case "library/toRead":
        ...
        return {...state, todo: [...state.todo, action.payload], results: newToRead};
        case "library/Read":
        ...
        return {...state, completed: [...state.completed, action.payload], results: newRead};
        case "library/remove":
            const remainingToRead = state.todo.filter(td => td.id !== action.payload);
        ...
        case 'book/open':
            if(state.todo.filter(td => td.id === action.payload).length > 0) {
                ...
                return {...state, openBook: state.openBook?.id === action.payload ? null : openBook, savedNotes: false}
            } else if(state.completed.filter(cp => cp.id === action.payload).length > 0) {
                const [openBook] = state.completed.filter(cp => cp.id === action.payload)
                return {...state, openBook: state.openBook?.id === action.payload ? null : openBook, savedNotes: false}
            }
            return {...state};
        case 'book/addNotes':
            return {...state, openBook: state.openBook ? {...state.openBook, notes: action.payload} : null}
        case 'list/addNotes':
            ...
            } else if(state.completed.filter(cp => cp.id === action.payload.id).length > 0) {
...
function LibraryProvider({children}: LibraryProviderProps) {
    ...
    const[{todo, completed, isActive, results, query, pageNumber, openBook, savedNotes}, dispatch] = useReducer(reducer, initialState, initializer);

    const getBooks = useCallback(async (topic: string, index: number)  => {

        let apiUrl = `/.netlify/functions/getBooks?topic=${topic}&index=${index}`;
        console.log(apiUrl)
        ...
        try {
            ...
            console.log({data})
            dispatch({type:"results/loaded", payload: data.map((book: any) => {
                return {
                            id: book.id,
                            ...
                            added: todo.filter(td => td.id === book.id).length > 0 || completed.filter(cp => cp.id === book.id).length > 0 ? true : false,
                            notes: ""
                        }
                    })
                })
    ...
    return(
        <LibraryContext.Provider value={{todo, completed, isActive, dispatch, results, query, pageNumber, getBooks, openBook, savedNotes}}>
            {children}
