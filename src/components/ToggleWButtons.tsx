...
export default function ToggleWButtons({info, isOpen, i, onClick, className, width = "500px", compressed = false} : ToggleElProps) {
    ...
    return(
        <li className="list-none" style={{width: `${width}`}}>
            <div aria-label={isOpen === i ? "Hide" : "Show"} onClick={() => onClick(i)} className={`p-4 bg-white text-base m-0 flex items-center justify-between gap-4 w-full ${className}`}>
                <div className="w-full flex items-center gap-4" key="title">
                ...
                <div className="p-4 text-sm"> 
                            <div className={compressed ? "max-h-40 overflow-y-scroll scrollbar": ""}>
                                ...
                                ? <div className="flex items-center justify-center gap-4 w-full px-4 pt-4">
                                    <Button label="Added!" rank="main" />
                                </div>
                                : <div className="flex items-center justify-center gap-4 w-full px-4 pt-4">
                                    <Button label="+ To Read" onClick={() => dispatch?.({type:"library/toRead", payload: info})}/>
                                    <Button label="+ Already Read" onClick={() => dispatch?.({type:"library/Read", payload: info})}/>
                                </div>}
                            </div>   
                </div>
            </div>         
