...
export default function APISearch({width = "300px"} : APISearchProps) {
    ...
    return(
        <div className="flex flex-col items-center justify-center bg-white" style={{width: `${width}`}}>
            <InputArea className={`bg-surface-container ${pageNumber === 0 ? "" : "border-b border-disabled"}`} label="Search" inputType="input" svg={Search} value={query} setValue={e => dispatch({type:"query/set", payload: e.target.value})} handleSubmit={handleSubmit} width="500px"/>
            <Accordion className="max-h-[453px] overflow-hidden overflow-y-auto scrollbar" infos={results} upperState={isOpen} upperSetState={toggleText} width={width}/>
            <div className={`flex items-center justify-center bg-surface border-t border-disabled w-full ${pageNumber === 0 ? "hidden" : ""}`}>
